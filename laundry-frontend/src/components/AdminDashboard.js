...
function AdminDashboard() {
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get(API_ENDPOINTS.ORDER_STATISTICS);
  ...
  const fetchEmployees = async () => {
    ...
    try {
      const response = await axios.get(API_ENDPOINTS.ORDER_EMPLOYEE_OVERVIEW);
  ...
  const handleUpdateOrder = async (updatedData) => {
    try {
      await axios.put(`${API_ENDPOINTS.ORDERS}/${selectedOrder.id}`, updatedData);
